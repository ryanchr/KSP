...
/*    for (int src_tmp=0; src_tmp < g.num_vertices()-1; src_tmp++)  
    ...
    while (k<Kvalue && !PQ_KSP_candidates_tau.empty() && iter<max_iter)
    {
        ...
        if (max_phy_dist==-1) // only for the first version of real data
            within_max_phy_dist = true;
...
/*    size_t root,threadnum;
    ...
    max_phy_dist = -1; // only for the first real data that "max_phy_dist = -1"
    max_phy_hops = 8;
