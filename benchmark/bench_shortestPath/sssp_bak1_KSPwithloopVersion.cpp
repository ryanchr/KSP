...
/*    for (int src_tmp=0; src_tmp < g.num_vertices()-1; src_tmp++)  
    ...
    size_t pk_last_id, tmp_id, deviation_id; // in tau
    size_t tmp_id_g, cur_deviation_id_g; // in tau

    stack<size_t> pk_vkt_nodes;
    tmp_id = dest_vit_tau->id();
...
/*    uint64_t curr_id_tau;
    do
    {
        curr_id_tau = dest_vit_tau->property().predecessor;
        pk_vkt_nodes.push(curr_id_tau);
        dest_vit_tau = tau.find_vertex(curr_id_tau);
    ...
    vertex_iterator_tau vit_tau_tmp;
    size_t k = 1;
    while (k<Kvalue && !PQ_KSP_candidates_tau.empty())
    {
        ...
        PQ_KSP_candidates_tau.pop();  // X = X - {p_k}
        while (!pk_vkt_nodes.empty())  // for each node at pk_vkt
        {
            //vector<uint64_t> neighborNodes_g; // A(v)
            vector<uint64_t> neighborNodes_tau, neighborNodes_tau_at_g; // A_Tk_(v)
            ...
            //g.find_vertex_out_neighborNodes(cur_deviation_id_g, neighborNodes_g);  // here only out neighbor is enouth since the input csv formats
...
/*#ifdef TOPKSP_DEBUG
        cout<<"Now the new pk_vkt_nodes is";
        pk_vkt_nodes_tmp = pk_vkt_nodes;
        cout<<"the values of pk_vkt_nodes at k="<<k<<" is ";
        while (!pk_vkt_nodes_tmp.empty())
        {
