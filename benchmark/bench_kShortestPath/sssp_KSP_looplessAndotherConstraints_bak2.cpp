...
cout<<"(min_cost,sum_distance,sum_hops)=("<<dest_vit_tau->property().min_cost<<","<<dest_vit_tau->property().sum_distance<<","<<dest_vit_tau->property().sum_hops<<")";
...
#ifdef TOPKSP_DEBUG
        cout<<endl;
        cout<<"iter just increased now iter="<<iter<<", and the pk_last_id="<<pk_last_id<<endl;
#endif
    ...
    cout<<"(the acutal number of running iterantion is "<<iter<<"):"<<endl;

    if (Kvalue!=KSPaths_lastID_tau.size())
    {
        cout<<"the input k value is too large, cannot find so many loopless paths!"<<endl;
        cout<<"the largest possible k is "<<KSPaths_lastID_tau.size()<<endl;
        assert(Kvalue==KSPaths_lastID_tau.size());
    }
...
void arg_init(argument_parser & arg)
{
    ...
    arg.add_arg("dest","1","root/dest vertex");
    arg.add_arg("Kvalue","5","root/K value");
    arg.add_arg("max_phy_dist","10","root/max_phy_dist");
    arg.add_arg("max_phy_hops","7","root/max_phy_hops");
}
//==============================================================//
