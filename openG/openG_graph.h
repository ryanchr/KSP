...
public:
    ...
    typedef typename std::tr1::shared_ptr<EPROP>  shared_eproperty_t;   // QS ?? purpose
    ...
    edge(uint64_t eid, uint64_t vid, const eproperty_t& prop):_eid(eid),_vid(vid)
    {
        ...
        *ptr = prop;
        _eproperty.reset(ptr);   // shared_eproperty_t _eproperty;    reset  ???  takes ownership of pointer?
    }
    ...
    {
        assert(_eproperty.use_count()>0);  // std::shared_ptr::use_count    Returns the number of different shared_ptr instances
        return *(_eproperty.get());        // .get()   Returns the stored pointer.
    }
...
protected:
    ...
    uint64_t _vid; // id of source/target vertex    // QS?? which one
    shared_eproperty_t _eproperty; // edge property   // typedef typename std::tr1::shared_ptr<EPROP>  shared_eproperty_t;
};



/*  from  OPENG_PROPERTY_H
...
public:
    ...
    bool find_vertex_inout_neighborNodes(uint64_t vid, std::vector<uint64_t>& neighborNodes)  // new
    {
        vertex_iterator viter = this->find_vertex(vid);
        for (edge_iterator eit=viter->in_edges_begin(); eit!=viter->in_edges_end(); eit++)   
        {
            neighborNodes.push_back(eit->target());
        }
        for (edge_iterator eit=viter->out_edges_begin(); eit!=viter->out_edges_end(); eit++) 
        {
            neighborNodes.push_back(eit->target());
        }
        return true;
    }
    bool find_vertex_out_neighborNodes(uint64_t vid, std::vector<uint64_t>& neighborNodes)  // new
    {
        vertex_iterator viter = this->find_vertex(vid);
        for (edge_iterator eit=viter->out_edges_begin(); eit!=viter->out_edges_end(); eit++) 
        {
    ...
    bool find_out_edge_2id(uint64_t src, uint64_t dest, edge_iterator& eiter)  //new
    {
        vertex_iterator src_iter = this->find_vertex(src);
        if (src_iter == _vertices.end()) return false;

        for (edge_iterator eit = src_iter->edges_begin(); eit != src_iter->edges_end(); eit++)  // for out edge
        {
            if (eit->target() == dest)
            {
                eiter = eit;
