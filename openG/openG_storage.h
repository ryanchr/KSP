...
template <class T>
class vector_storage : public std::vector<T>   //// QS??  why not just vector
{
...
public:
    ...
    typedef typename base_t::value_type value_type;      //// QS??  value_type
...
template <class T>
class indexed_vector_storage : public std::vector<T>   //// QS??  indexed_
{
    typedef std::vector<T> base_t;
...
public:
    ...
    {
        if (iter == this->end()) return this->end();
        _index.erase(iter->id());           //   iter->id()  ??   iter- iterator  new_iterator,  Line 15 ??
        invalidate(iter);
        iter++;
        return iter;
    }
